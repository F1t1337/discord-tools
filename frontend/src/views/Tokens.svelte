<script>
  ...
  async function exportTokens(destination = 'telegram') {
    ...
    try {
      await api('/tokens/export' + (destination === 'tskupka' ? '/tskupka' : ''), { method: 'POST', body: '{}' });
...
<PageHeader title="Токены" description="Загрузка токенов в обработку и выгрузка готовых.">
  {#snippet actions()}
    <button class="button secondary" onclick={() => exportTokens()} disabled={busy || exportStarting || !workerRunning}>
      <Icon name="upload" size={16} /> Обычная выгрузка
    </button>
    <button class="button primary" onclick={() => exportTokens('tskupka')} disabled={busy || exportStarting || !workerRunning || !job?.tskupka_configured}>
      Выгрузить в Tskupka
    </button>
  {/snippet}
</PageHeader>
...
<div class="settings-grid">
  ...
  <article class="panel" use:reveal={1}>
    ...
    <p class="muted">«Обычная выгрузка» сохраняет файл и отправляет его в Telegram. «Выгрузить в Tskupka» проверяет готовые токены и сразу передаёт их в Tskupka, без отдельного подтверждения. Статус задачи обновляется кнопкой рядом с ней.</p>
      ...
      <div class="list">
        {#each history as item (item.id)}
          <div class="list-item export-history-item">
            <div><strong>#{item.id} · {number(item.count)} шт.</strong><div class="meta">{date(item.created_at)} · {deliveryLabel(item.delivery)}</div>
              <p class="meta">{tskupkaLabel(item.tskupka)}</p>
              {#if item.tskupka?.error}<p class="form-error">{item.tskupka.error}</p>{/if}
              {#if item.tskupka?.summary?.amount_paid != null}<p class="meta">Начислено по данным Tskupka: {number(item.tskupka.summary.amount_paid)}</p>{/if}
            </div>
