...
<div class="settings-grid">
  ...
  <article class="panel" use:reveal={1}>
    ...
    <p class="muted">«Обычная выгрузка» сохраняет файл и отправляет его в Telegram. «Выгрузить в Tskupka» проверяет готовые токены выбранной закупки и передаёт их без отдельного подтверждения. Предварительная сумма (initial_total) подтягивается автоматически, после чего опрос прекращается. Кнопка «Обновить Tskupka» подтягивает актуальную сумму: итоговую (final_total) после финализации, иначе предварительную (initial_total) — можно и вручную поправить расхождения.</p>
