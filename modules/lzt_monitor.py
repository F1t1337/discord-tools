...
class LZTMonitor:
    ...
    def __init__(self, api_token: str, check_interval: int = 60, min_balance_alert: float = 100):
        """
    ...
    def get_new_purchases(self) -> List[Dict]:
        ...
        for account in accounts:
            ...
            buy_date = account.get('buy_date')

            # Если это первый запуск, устанавливаем время последней проверки
            if self.last_check_time is None:
                self.last_check_time = datetime.now().timestamp()
                # При первом запуске не обрабатываем старые покупки
                self.processed_items.add(item_id)
                continue

            # Проверяем, что покупка новая
            if buy_date and buy_date > self.last_check_time:
                # Извлекаем токен
